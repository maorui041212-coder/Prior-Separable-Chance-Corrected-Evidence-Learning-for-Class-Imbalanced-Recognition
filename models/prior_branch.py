"""
Prior branch for CCEL-Net.

It estimates prior-induced bias logits:
    b = sum_k w_k log(pi_k)
where pi_k may include global prior, training mini-batch prior, and prediction-prior EMA.

Important implementation details
--------------------------------
1. Validation/test must not use target-derived batch prior. This branch only uses
   batch prior when ``allow_target_prior=True`` and target is provided. In CCELNet,
...
class PriorBranch(nn.Module):
    ...
    def _add_prior_term(self, terms, weights, prior: Tensor, weight: float, name: str) -> None:
        if weight == 0.0:
            return
    ...
    def forward(
        ...
        *,
        ...
        if self.use_global_prior:
            self._add_prior_term(terms, weights, global_prior, self.global_prior_weight, "global")
        ...
        if batch_prior_available:
            ...
            self._add_prior_term(terms, weights, batch_prior, self.batch_prior_weight, "batch")
        else:
            batch_prior = torch.empty(0, device=device, dtype=dtype)

        if self.use_pred_prior_ema:
            self._add_prior_term(terms, weights, pred_prior, self.pred_prior_weight, "pred")
        ...
        prior_logits = prior_logits * self.prior_logit_scale
